...
def build_prompt(genre, theme, style, magazine, length_key, passion_key):
    ...
    # Knipe's secret trick: insert one long obscure word
    obscure_words = [
...
def run_machine(genre, theme, style, magazine, length_key, passion_key):
    client = anthropic.Anthropic()

    prompt = build_prompt(genre, theme, style, magazine, length_key, passion_key)

    _, (length_label, length_desc) = length_key, LENGTH_BUTTONS[length_key]
...
def save_story(story_text, genre, theme, magazine):
    filename = f"grammatizator_{genre}_{theme}_{magazine}.txt".replace(" ", "_").lower()
    with open(f"/mnt/user-data/outputs/{filename}", "w") as f:
        f.write("THE GREAT AUTOMATIC GRAMMATIZATOR\n")
        f.write(f"Genre: {genre} | Theme: {theme} | Magazine: {magazine}\n")
        f.write("=" * 64 + "\n\n")
        f.write(story_text)
    return filename
...
def main():
    ...
    while True:
        ...
        if go == "y":
            run_machine(genre, theme, style, magazine, length_key, passion_key)
